...
df = pd.read_csv(./train/corpus.txt', sep=',', engine='python', header=0, index_col=False)

train_gu = df['guja_2_hindi'].values.tolist()
train_en = df['target'].values.tolist()

SENT_MODEL_PATH = './dataset/train/sentencepiece/'
hi_model_path = SENT_MODEL_PATH + 'hi/tokenizer_32000'
en_model_path = SENT_MODEL_PATH + 'en_hi/tokenizer_32000'
gu_tokenizer = spm.SentencePieceProcessor(model_file=f'{hi_model_path}.model')
en_tokenizer = spm.SentencePieceProcessor(model_file=f'{en_model_path}.model')


def data_process(src, tgt, src_tokenizer, tgt_tokenizer):
    data = []
    for (raw_src, raw_tgt) in zip(src, tgt):
        if type(raw_tgt) != float: # to get rid of the empty sentence
            src_tensor = torch.tensor(src_tokenizer.encode(raw_src), dtype=torch.long)
            tgt_tensor = torch.tensor(tgt_tokenizer.encode(raw_tgt), dtype=torch.long)
            data.append((src_tensor, tgt_tensor))

    return data


train_data = data_process(train_gu, train_en, gu_tokenizer, en_tokenizer)
...
def main():
    ...
    torch.save({
        ...
        'loss': train_loss}, 'model_checkpoint.tar')
